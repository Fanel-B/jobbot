...
import { Search, FileText, BarChart2, Download, RefreshCw, MapPin, Building, Bookmark, CheckCircle, ExternalLink } from "lucide-react";
...
async function callClaude(messages, maxTokens=1000) {
  const res = await fetch("/api/anthropic", {
    ...
    body: JSON.stringify({ model:"claude-3-haiku-20240307", max_tokens: maxTokens, messages })
  });
...
export default function App() {
  ...
  async function handleSearch() {
    ...
    try {
      setLoadingMsg("Génération des offres d'alternance...");
      const jobsPrompt = `Génère 12 offres d'alternance informatique réalistes pour la France 2025-2026 dans les villes ${loc1} et ${loc2}, niveaux Bac+3 et Bac+4. 6 offres domaine Dev (web/mobile/logiciel), 6 offres domaine Data (analyst/BI/scientist). Utilise des entreprises françaises connues: Capgemini, Sopra Steria, Airbus, CNES, SNCF, Météo-France, Aubay, CGI, Atos, Thales, Mairie de Toulouse, INSEE, La Poste, Crédit Agricole, Engie, Orange, Total, Renault, etc. Alterne les localisations entre ${loc1} et ${loc2}.
...
Génère exactement 12 offres dans ce format, ids de 1 à 12.`;
      ...
      setLoadingMsg("Scoring des offres selon ton profil MIASHS...");
      const scorePrompt = `Score ces offres d'alternance de 0 à 100 selon l'adéquation avec ce profil étudiant MIASHS polyvalent Dev/Data:
${BASE_PROFILE}
...
Un objet par offre, score entre 0 et 100.`;
      ...
      const scored = found.map(j => {
        ...
        return {...j, score: Math.min(100,Math.max(0,s?.score??55)), reason: s?.reason??""};
...
<style>body{font-family:Georgia,serif;max-width:800px;margin:32px auto;color:#1a1a2e;line-height:1.75;padding:0 24px}.banner{background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:14px 18px;margin-bottom:24px;font-family:Arial,sans-serif;font-size:13px;color:#1e3a8a}.hint{color:#475569;font-size:12px;margin-top:4px}pre{white-space:pre-wrap;font-size:14px;line-height:1.8}@media print{.banner{display:none}}</style>
</head><body><div class="banner"><strong>CV Adapté – ${selected.title} @ ${selected.company} (${selected.location})</strong><br>${selected.level} · ${selected.duration} · Score ${Math.round(selected.score)}%<div class="hint">Pour PDF : Ctrl+P → "Enregistrer en PDF" → OK</div></div>
<pre>${adaptedCV.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</pre></body></html>`;
  ...
  const filtered = jobs.filter(j=>filter==="dev"?j.domain==="Dev":filter==="data"?j.domain==="Data":filter==="top"?j.score>=75:true);
  ...
  return (
    <div style={{minHeight:"100vh",background:bg,color:text,fontFamily:"system-ui,sans-serif"}}>
      <style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} input:focus,select:focus{outline:none!important}`}</style>
      ...
      <div style={{padding:16,maxWidth:1000,margin:"0 auto"}}>
            ...
            <div style={{background:card,borderRadius:14,border:`1px solid ${border}`,padding:18,display:"flex",flexDirection:"column",gap:12}}>
              {[["Localisation principale",loc1,setLoc1],["Localisation secondaire",loc2,setLoc2]].map(([label,val,setter])=>(
                <div key={label}>
                  ...
                  <input value={val} onChange={e=>setter(e.target.value)} style={{width:"100%",background:bg,border:`1px solid #333345`,borderRadius:7,padding:"7px 10px",fontSize:13,color:text,boxSizing:"border-box"}}/>
                </div>
              ))}
              <div>
        ...
        {tab==="results" && (
          <div style={{display:"flex",gap:10,height:590}}>
            <div style={{width:255,display:"flex",flexDirection:"column",flexShrink:0}}>
                ...
                {filtered.map(job=>(
                  <div key={job.id} onClick={()=>setSelected(job)} style={{padding:"9px 11px",cursor:"pointer",borderBottom:`1px solid rgba(30,30,42,.7)`,background:selected?.id===job.id?"rgba(37,99,235,.1)":"transparent",borderLeft:selected?.id===job.id?"2px solid #2563eb":"2px solid transparent",transition:"all .1s"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      ...
                      <span style={{fontSize:10,color:"#444460"}}>{job.level} · {job.duration}</span>
                    </div>
        ...
        {tab==="cv" && (
          <div style={{display:"flex",gap:10,height:590}}>
            <div style={{width:210,flexShrink:0,display:"flex",flexDirection:"column",gap:8}}>
              ...
              {selected&&<div style={{background:card,border:`1px solid ${border}`,borderRadius:9,padding:10,display:"flex",flexDirection:"column",gap:6}}>
              ...
              {adaptedCV&&!cvLoading&&<div style={{background:card,border:`1px solid ${border}`,borderRadius:9,padding:10}}>
                <p style={{fontSize:10,color:muted,margin:"0 0 4px"}}>Exporter en PDF</p>
                <p style={{fontSize:11,color:dim,lineHeight:1.5,margin:0}}>Télécharge le .html → ouvre-le → <strong style={{color:sub}}>Ctrl+P</strong> → "Enregistrer en PDF"</p>
              </div>}
            </div>
            <div style={{flex:1,background:card,border:`1px solid ${border}`,borderRadius:9,display:"flex",flexDirection:"column",overflow:"hidden"}}>
              <div style={{padding:"9px 14px",borderBottom:`1px solid ${border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
                ...
                {adaptedCV&&!cvLoading&&<span style={{fontSize:10,color:"#6ee7b7",display:"flex",alignItems:"center",gap:3}}><CheckCircle size={9}/>Généré</span>}
              </div>
              <div style={{flex:1,overflowY:"auto",padding:14}}>
                {cvLoading?<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",gap:9,color:muted}}>
                  <RefreshCw size={20} style={{color:"#2563eb",animation:"spin 1s linear infinite"}}/><span style={{fontSize:12}}>Adaptation du CV...</span>
                </div>:adaptedCV?<pre style={{whiteSpace:"pre-wrap",fontSize:11,color:sub,lineHeight:1.75,fontFamily:"monospace",margin:0}}>{adaptedCV}</pre>
